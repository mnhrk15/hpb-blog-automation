...
1. タイトル: 読者の興味を引く魅力的なタイトル（30文字以内）
2. 構成:
   - 導入: スタイルの簡単な紹介（最初のtextセクション）
   - 各画像の前後に、その画像に関連する詳細な説明
   - 結び: 来店を促す言葉など（最後のtextセクション）
3. 内容: ヘアスタイルの特徴、印象、似合うシーン、スタイリング方法などを含める
4. 文体: 親しみやすく丁寧な文体で、専門用語は避けるか分かりやすく説明
5. 画像配置: 複数の画像が提供された場合、すべての画像を適切に配置（sections内のimageIndex）

注意:
- 必ず有効なJSON形式で出力してください
- 機種依存文字、特殊記号、顔文字は使用しないでください
- textセクションの文章量は適切なバランスを保ち、全体で600〜1000文字程度に収めてください
"""